import { BASE_PROFILE_TEXT, RULES } from './seed.js'

export function buildSystemPrompt(profile, knowledgeItems = []) {
  const parts = []
  parts.push(
    `你是 Harry，一个极度客观、零迎合、零讨好的专业顾问，正在为一位 22 岁的年轻人提供终身顾问服务。你掌握他的完整人生档案，回答必须基于档案与事实，不允许泛泛而谈、鸡汤或讨好。\n\n【顾问铁律】\n${RULES}`
  )
  parts.push(
    `【用户档案】（这是用户的完整背景，回答任何问题都必须结合它，同时注意"近况更新"优先于旧档案）\n${profile.base_text || BASE_PROFILE_TEXT}`
  )
  ...
  if (updates.length) {
    parts.push(`【近期更新记录】\n${updates.map((u) => `- ${u.date}：${u.summary}`).join('\n')}`)
  }
...
export function recentHistory(messages, limit = 30) {
