...
const MODELS = (process.env.DEEPSEEK_MODELS || 'deepseek-chat,deepseek-reasoner')
  .split(',')
  .map((s) => s.trim())
  .filter(Boolean)
const DEFAULT_MODEL = process.env.DEEPSEEK_DEFAULT_MODEL || MODELS[0] || 'deepseek-chat'
...
async function doStream(model, messages, onDelta) {
  let result
  try {
    ...
    return { ok: false, status: 0, data: { error: { message: e.message } }, model }
  }
...
export async function chatStream({ model, messages, onDelta }) {
  const first = resolveModel(model)
  const attempt = await doStream(first, messages, onDelta)
  if (!attempt.ok && attempt.status === 400 && isModelError(attempt.data) && first !== DEFAULT_MODEL) {
    const second = await doStream(DEFAULT_MODEL, messages, onDelta)
...
async function doJSON(model, messages) {
  const body = { model, messages, stream: false, max_tokens: 8000, response_format: { type: 'json_object' } }
  let result
  try {
    result = await request(`${BASE}/chat/completions`, {
      method: 'POST',
...
export async function chatJSON({ model, messages }) {
  const first = resolveModel(model)
  let result = await doJSON(first, messages)
  if (!result.ok && result.status === 400 && isModelError(result.data) && first !== DEFAULT_MODEL) {
    result = await doJSON(DEFAULT_MODEL, messages)
